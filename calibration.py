...
def color_calibration(in_img, ref_data, show_img=False):
    ...
    for l_y in range(ref_data[0][0][1]):
        for l_x in range(ref_data[0][0][0]):
            ...
            rec = (cv.rectangle(rec[0], top_left, bottom_right, (0, 0, 255), 2), rec[1])
            grid_sample = in_img[0][top_left[1]:bottom_right[1], top_left[0]:bottom_right[0]]
            avg_col = image_utilities.get_average_color((grid_sample, in_img[1]))
            ...
            ref_xy = image_manipulation.convert_color((ref_data[1][l_x][l_y], in_img[1]), 'xy')[0]
            avg_xy = image_manipulation.convert_color((avg_col, in_img[1]), 'xy')[0]
    ...
    fit_data[0] = np.average(sample_data[0])
    fit_data[1] = sample_data[0].min(initial=1000.0)
    fit_data[2] = sample_data[0].max(initial=0.0)
    ...
    if show_img:
        image_utilities.show_image("Calibrated ColorChecker", image_manipulation.scale_image(rec)[0], False)
    else:
        ...
        # plot_domain = settings.input_lab_domain
        plot_domain = ((0, -60, -60), (100, 60, 60))
        edges = []
        step_size = 2
        for o in range(3):
...
def image_uniformity(in_img):
    ...
    img_d = img_d.astype(main_script.bit_type[0])
